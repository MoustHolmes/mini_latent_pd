"""Utilities for converting mdCATH HDF5 data to Minifold Atom37 format."""
...
def decode_bytes_array(arr):
    """Decode an array of bytes/strings from HDF5 to a numpy string array."""
...
def parse_atom_names_from_pdb(pdb_text: str) -> np.ndarray:
    """Parse atom names from PDB-format text (columns 12-16), first MODEL only."""
    atom_names = []
...
def build_scatter_indices(heavy_atom_names, heavy_resids):
    """
    Pre-compute indices for vectorized scattering of flat heavy-atom coords
    into (num_residues, 37, 3) atom37 format.

    Returns:
        valid_heavy_idx: indices into the heavy-atom array for recognized atoms
        res_scatter: corresponding 0-based residue index for each valid atom
        atom37_scatter: corresponding atom37 slot for each valid atom
    """
    ...
    for i, (aname, resid) in enumerate(zip(heavy_atom_names, heavy_resids)):
        if aname in residue_constants.atom_order:
            valid_heavy_idx.append(i)
            res_scatter.append(resid_to_seq_idx[resid])
            atom37_scatter.append(residue_constants.atom_order[aname])
...
def aatype_from_resnames(resnames, resids):
    ...
    for seq_idx, resid in enumerate(unique_resids):
        ...
        res_1_letter = residue_constants.restype_3to1.get(std_resname, "X")
        aatype[seq_idx] = residue_constants.restype_order.get(res_1_letter, 20)
...
def compute_atom37_mask(aatype):
    ...
    for i, aa in enumerate(aatype):
        if aa < len(residue_constants.restypes):
            resname_3 = residue_constants.restype_1to3[residue_constants.restypes[aa]]
            for atom_name in residue_constants.residue_atoms[resname_3]:
                if atom_name in residue_constants.atom_order:
                    mask[i, residue_constants.atom_order[atom_name]] = 1.0
    return mask
